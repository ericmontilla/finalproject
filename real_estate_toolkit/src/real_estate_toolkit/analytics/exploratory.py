...
class MarketAnalyzer:
    ...
    def clean_data(self) -> None:
        ...
        for col in data.columns:
            if data[col].dtype == pl.Utf8:  # String (categorical) columns
                # Usamos fill_null() para cadenas (rellenar con "Unknown")
                data = data.with_columns(data[col].fill_null("Unknown").alias(col))
            elif data[col].dtype in [pl.Float64, pl.Int64]:  # Numeric columns
                # Usamos fill_nan() para valores numéricos (por ejemplo, con la mediana)
                median = data.select(pl.median(col)).row(0)[0]
                data = data.with_columns(data[col].fill_nan(median).alias(col))  # Usamos fill_nan() para numéricos
        ...
        # Eliminar las columnas de tipo 'object' (cadenas) que no son relevantes
        irrelevant_columns = ["Alley", "PoolQC", "Fence", "MiscFeature"]  # Suponiendo que estas no son relevantes
        for col in irrelevant_columns:
            if col in object_columns:
                object_columns.remove(col)

        data = data.drop(object_columns)  # Eliminar columnas no relevantes de tipo 'object'

        # Step 5: Convert categorical variables to dummies (binary variables)
        for col in categorical_columns:
            if col in data.columns:
                # Para crear dummies, iteramos por cada valor único en la columna
                categories = data[col].unique().to_list()
                for category in categories:
                    # Usamos pl.when() para crear una columna dummy para cada categoría
    ...
    def generate_price_distribution_analysis(self) -> pl.DataFrame:
        ...
        plot_file = os.path.join(output_dir, "sale_price_distribution.html")
    ...
    def feature_correlation_heatmap(self, variables: list) -> None:
     ...
     """
     try:
         # Verificar que las variables pasadas están presentes en los datos
    ...
    def create_scatter_plots(self) -> dict:
        ...
        output_path = "src/real_estate_toolkit/analytics/outputs/"
        if not os.path.exists(output_path):
            os.makedirs(output_path)

        fig1.write_html(os.path.join(output_path, "Sale_price_vs_GrLivArea.html"))
        fig2.write_html(os.path.join(output_path, "Sale_price_vs_Year_built.html"))
        fig3.write_html(os.path.join(output_path, "Sale_price_vs_Overall_quality.html"))
...
if __name__ == "__main__":
    data_path = "src/real_estate_toolkit/data/input/train.csv"  # Adjust to your actual file path
...
if __name__ == "__main__":
    data_path = "src/real_estate_toolkit/data/input/train.csv"  # Ajusta la ruta a tu archivo CSV
    analyzer = MarketAnalyzer(data_path=data_path)
    print("Cleaning data...")
    analyzer.clean_data()
    print("Data cleaning complete.")
